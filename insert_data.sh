...
TEAM_ID_0=$($PSQL "SELECT team_id FROM teams WHERE name ='$OPPONENT'")

if [[ -n $TEAM_ID_W || -n $TEAM_ID_0 ]]
then
if [[ $YEAR != "year" ]]
then
INSERT_GAMES=$($PSQL "INSERT INTO games(year, round, winner_id, opponent_id, winner_goals, opponent_goals) VALUES 
($YEAR, '$ROUND', $TEAM_ID_W, $TEAM_ID_0, $WINNER_GOALS, $OPPONENT_GOALS);")
